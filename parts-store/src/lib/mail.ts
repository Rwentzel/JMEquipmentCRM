...
import { SUPPORT_FORMS, isSupportKind, type SupportForm } from "@/lib/supportRequest";
...
/**
 * A Support Hub request. Named by its form, and its answers listed under the
 * labels from our own spec — indented, so a value can never pass for one of
 * the contact lines the desk reads at the top.
 */
function formatSupportEmail(rfq: StoredRfq, form: SupportForm): { subject: string; text: string } {
  const c = rfq.contact;
  const lines = [`New ${form.label.toLowerCase()} ${rfq.ref}`, `Received: ${rfq.createdAt}`, ""];
  if (c.company) lines.push(`Company:  ${oneLine(c.company)}`);
  if (c.name) lines.push(`Contact:  ${oneLine([c.name, c.lastName].filter(Boolean).join(" "))}`);
  if (c.email) lines.push(`Email:    ${oneLine(c.email)}`);
  if (c.phone) lines.push(`Phone:    ${oneLine(c.phone)}`);
  if (c.serial) lines.push(`Serial:   ${oneLine(c.serial)}`);
  const details = Object.entries(rfq.details ?? {});
  if (details.length) {
    lines.push("", "Details:");
    for (const [label, value] of details) lines.push(`  ${oneLine(label)}: ${oneLine(value)}`);
  }
  ...
  lines.push(
    ...
    c.email
      ...
      : "This notification was generated by the parts store; the customer gave a phone number only.",
  );
  return {
    subject: `[${form.tag}] ${rfq.ref} — ${oneLine(c.company || c.name || c.serial || c.email)}`,
    text: lines.join("\n"),
  };
}

export function formatRfqEmail(rfq: StoredRfq): { subject: string; text: string } {
  if (rfq.kind && isSupportKind(rfq.kind)) return formatSupportEmail(rfq, SUPPORT_FORMS[rfq.kind]);
  const c = rfq.contact;
  // "Send a Message" is its own button on the storefront, and it submits with
  // no line items. Announcing it as a quote request — under an empty "Items:"
  // heading — describes it as something the customer did not do, and buries
  // the one thing that is actually in it.
  const enquiry = rfq.items.length === 0;
  const lines = [
    enquiry
  ...
  lines.push(
    "",
    `Work this ${enquiry ? "message" : "request"} in the ops desk: ${siteUrl()}/ops (ref ${rfq.ref}).`,
    ...(enquiry
      ? []
      : [
          // Paste this into the customer's order confirmation: it opens the
          // storefront with their reference prefilled so they can reorder
          // the same parts in one step (their email is still required).
          `Customer reorder link for the confirmation: ${siteUrl()}/?reorder=${rfq.ref}`,
        ]),
    "This notification was generated by the parts store; reply goes to the customer address above.",
  );
  return {
    subject: `[${enquiry ? "MESSAGE" : "RFQ"}] ${rfq.ref} — ${oneLine(c.company)}${rfq.freight ? " (freight)" : ""}`,
    text: lines.join("\n"),
  };
}
...
export async function sendRfqNotification(rfq: StoredRfq): Promise<boolean> {
  ...
  try {
    ...
    await transporter.sendMail({
      ...
      // A phone-only service request has no address to reply to.
      ...(rfq.contact.email ? { replyTo: rfq.contact.email } : {}),
      subject,
      text,
    });
    audit("mail_sent");
