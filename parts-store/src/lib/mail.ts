...
export function formatRfqEmail(rfq: StoredRfq): { subject: string; text: string } {
  ...
  // A Support Hub request names its own kind — a manual request announced as
  // "a message" makes the desk read the whole body to learn what it is.
  const support = rfq.requestType ? SUPPORT_SPECS[rfq.requestType] : null;
  const lines = [
    support
      ? `New ${support.label.toLowerCase()} ${rfq.ref}`
      : enquiry
    ...
    "",
    `Company:  ${oneLine(c.company) || "(not given)"}`,
    `Contact:  ${oneLine([c.name, c.lastName].filter(Boolean).join(" ")) || "(not given)"}`,
    `Email:    ${oneLine(c.email) || "(not given)"}`,
  ];
  ...
  if (c.wantsAccount === false && !support) lines.push("Account:  customer opted OUT of an account");
  if (support && rfq.requestType) {
    const details = supportDetailLines(rfq.requestType, rfq.fields ?? {});
    if (details.length) lines.push("", "Details:", ...details.map((d) => "  " + oneLine(d)));
  }
  ...
  lines.push(
    "",
    `Work this ${enquiry && !support ? "message" : "request"} in the ops desk: ${siteUrl()}/ops (ref ${rfq.ref}).`,
    ...(enquiry
      ? []
      : [
  ...
  return {
    subject: support
      ? `[${support.flag}] ${rfq.ref} — ${support.label} — ${oneLine(c.company) || oneLine(c.name) || oneLine(c.email)}`
      : `[${enquiry ? "MESSAGE" : "RFQ"}] ${rfq.ref} — ${oneLine(c.company)}${rfq.freight ? " (freight)" : ""}`,
    text: lines.join("\n"),
  };
}
