  ...
  const results = useMemo<Part[]>(
    () =>
      D.parts.filter((p) => {
        ...
        const haystack = [p.sku, p.name, p.category, p.fitment, p.description, ...(p.keywords ?? [])]
          .filter(Boolean)
          .join(" ")
          .toLowerCase();
        const inQ = !dq || haystack.includes(dq.toLowerCase());
        return inCat && inQ;
      }),
    [dq, cat],
  );
      ...
      <div className="ps-wrap">
        ...
        <div className="ps-search">
          <input
            ...
            placeholder="Search — e.g. JME-GMC-BLD-001, blade, belt, bearing"
        ...
        <div className="ps-parts">
          {results.map((p) => (
            <div className="ps-part" key={p.sku}>
              ...
              {p.description && <p className="ps-part__desc">{p.description}</p>}
              <div className="ps-part__tags">
                <Tag>{p.cat}</Tag>
                {p.category && <Tag>{p.category}</Tag>}
              </div>
              {p.fitment && <div className="ps-part__fit">Fits: {p.fitment}</div>}
