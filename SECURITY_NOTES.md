...
## Rate limiting
- **Caller identity:** taken from a proxy-set header (`Fly-Client-IP`, `CF-Connecting-IP`, `True-Client-IP`, `X-Real-IP`) in preference to anything the client sent, falling back to the **last** `X-Forwarded-For` hop — the entry our own proxy appended. Previously the **first** XFF entry was used, which is the one value in the chain a client fully controls: rotating it per request let 1000 of 1000 submissions through a 5-per-minute limit. Behind no proxy at all nothing here is trustworthy and the limiter degrades to per-process.
- **Memory:** buckets are capped and evicted (finished windows first, then oldest-inserted) down to a low-water mark. They were previously never released, so a rotated header grew the map for the life of the process — 500k unique keys measured 147 MB of heap on a deployment the runbook specifies as a single small instance. Evicting a live bucket does grant that caller a fresh allowance; that only happens under a flood large enough to fill the map, where the alternative is an OOM.
- **Still sandbox-grade:** per-process and lost on restart. Production wants a shared store or edge rate limiting, as below.
