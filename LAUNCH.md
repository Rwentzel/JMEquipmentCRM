...
- **After any design or CSS change**: run the accessibility audit against a
  ...
  it skipped rather than reporting a clean run over half the app. With the
  token it also mints a real quote (a storefront request turned into a draft
  the way the desk does it) and audits the customer's share-link page, desktop
  and phone; without it, `A11Y_QUOTE_PATH=/q/<id>/<token>` — a real link out
  of the builder — covers that page. Non-zero exit = something on screen is
  unreadable or unlabelled.
...
Three more links open the store at a particular place. None of them reads
stored data, so all are safe in a signature that goes to every customer:

- **Search link** — `/?q=bearing` opens the storefront with the catalogue
  already filtered to that word, SKU or machine name. Good for "find your
  wear parts here" lines.
- **Machine link** — `/machines?m=JME-VCS12-75` opens the Machine Platform
